...
#include "PCMeshMergeFunctionLibrary.generated.h"
...
struct POWERCRYSTALS_API FPCSkelMeshMergeSectionMapping_BP
{
	GENERATED_BODY()
...
*/
USTRUCT(BlueprintType)
struct POWERCRYSTALS_API FPCSkelMeshMergeUVTransform
{
	GENERATED_BODY()
...
*/
USTRUCT(BlueprintType)
struct POWERCRYSTALS_API FPCSkelMeshMergeUVTransformMapping
{
	GENERATED_BODY()

	/** For each UV channel on each mesh, how the UVS should be transformed. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Mesh Merge Params")
	TArray<FPCSkelMeshMergeUVTransform> UVTransformsPerMesh;
};
...
*/
USTRUCT(BlueprintType)
struct POWERCRYSTALS_API FPCSkeletalMeshMergeParams
{
	GENERATED_BODY()

	FPCSkeletalMeshMergeParams()
	{
		MeshSectionMappings = TArray<FPCSkelMeshMergeSectionMapping_BP>();
		UVTransformsPerMesh = TArray<FPCSkelMeshMergeUVTransformMapping>();
		StripTopLODS = 0;
	...
	TArray<FPCSkelMeshMergeSectionMapping_BP> MeshSectionMappings;

	// An optional array to transform the UVs in each mesh
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<FPCSkelMeshMergeUVTransformMapping> UVTransformsPerMesh;
 ...
 */
UCLASS()
class POWERCRYSTALS_API UPCMeshMergeFunctionLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	*/
	...
	static class USkeletalMesh* MergeMeshes(const FPCSkeletalMeshMergeParams& Params);
